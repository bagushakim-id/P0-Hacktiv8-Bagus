...
(select school, max(salary) salary
from teachers
group by school) b
where b.school = a.school and b.salary = a.salary;
...
select c.teachers_id, t.first_name, t.last_name, t.school, t.hire_date, c.name courses_name
from teachers t
join courses c
...
select t.id, t.first_name, t.last_name, count(c.name) total_courses
from teachers t
join courses c
on t.id = c.teachers_id
group by t.id
